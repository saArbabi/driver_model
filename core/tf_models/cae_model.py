...
class Decoder(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        step_cond_m = self.axis2_conc([act_mlon, act_mlat, act_y, act_f, act_fadj, enc_h, enc_h, enc_h])
        ...
        for step in tf.range(steps_n):
            tf.autograph.experimental.set_loop_options(shape_invariants=[
                            ...
                            (step_cond_m, tf.TensorShape([None,None,305])),
                            (step_cond_y, tf.TensorShape([None,None,204])),
            ...
            """Merger vehicle long
            """
            # outputs, state_h_m, state_c_m = self.lstm_layer_m(enc_h, \
            #                                     initial_state=[state_h_m, state_c_m])
            outputs, state_h_m, state_c_m = self.lstm_layer_m(self.axis2_conc([
                                    step_cond_m, enc_h]),
                                    initial_state=[state_h_m, state_c_m])
            ...
            """Yielder vehicle
            """
            outputs, state_h_y, state_c_y = self.lstm_layer_y(self.axis2_conc([
                                    step_cond_y, enc_h]),
