...
def create_dataset(data, x_len, y_len):
    ...
    for i in range(len(data) - x_len - y_len):
        # conditioning_param = np.random.choice(range(4)) # to test if I can condition the model

        x_sequence = data.iloc[i:(i + x_len)].values
        y_input_sequence = data.iloc[(i + x_len):(i + x_len + y_len)].values
        y_target_sequence = data.iloc[(i + x_len + 1):(i + x_len + y_len + 1)].values
...
xs_val, ys_input_val, _ = create_dataset(test, x_len, y_len)
...
decoded_seq = decode_sequence(input_seq, ys_input_val[0][0][0], step_n)
np.array(decoded_seq).flatten()
# %%
# encoder_model.predict(input_seq)
x_true = test.sine.values[0:100]
len(x_true)
plt.plot(range(100),x_true)
plt.plot(range(x_len,x_len+y_len),ys_input_val[0])
plt.plot(range(x_len,x_len+step_n),np.array(decoded_seq).flatten())
# %%
