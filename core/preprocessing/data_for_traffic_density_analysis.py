...
# %%
def pickup_episodes(validation_set, min_dx, max_dx, episode_n):
    # traffic density is roughly assumed to be indicated by the size of the merge gap

    if max_dx == 70:
        potential_episodes = validation_set[np.where(
                                            (validation_set[:, 6]<max_dx) &
                                            (validation_set[:, 6]>min_dx)
                                            )[0]][:,0]
        ...
        potential_episodes = validation_set[np.where(
                                            (validation_set[:, 6]<max_dx) &
                                            (validation_set[:, 6]>min_dx) &
                                            (validation_set[:, 12]<max_dx) &
                                            (validation_set[:, 12]>min_dx)
                                            )[0]][:,0]
...
# %%
low_density_episodes = pickup_episodes(validation_set, min_dx=23, max_dx=70, episode_n=50)
medium_density_episodes = pickup_episodes(validation_set, min_dx=10, max_dx=23, episode_n=50)
high_density_episodes = pickup_episodes(validation_set, min_dx=0, max_dx=10, episode_n=50)
